...
'''
@Description:       :
domain mining for LCC Verb TroFi VUA-Verb
...
if __name__ == '__main__':
    dataset = 'VUA_Verb' # TroFi LCC_Verb or VUA_Verb
    ...
    for file_name in dataset_to_file[dataset]:
        ...
        for sample_i in range(len(output_dataset), len(input_dataset)):
            ...
            verb_parser_dict_clean_ = split_key(verb_parser_dict)
            verb_parser_dict_clean = {}
            # 1. target_token 过滤 ：在原句中找不到的不要 (有些是因为有连字符所以找不到), target word和focus verb的前后顺序需要在原句中找到才能获得
            for key_ in list(verb_parser_dict_clean_.keys()):
                tokens_key = []
                for token_key in verb_parser_dict_clean_[key_]['tokens']:

                    target_role_tokens = token_key.split('_')
                    ...
                    flag_token_key = False

                    for index_target_role in range(len(words_lemma) - len(target_role_tokens) + 1):
                        if words_lemma[index_target_role: index_target_role + len(target_role_tokens)] == target_role_tokens_lemma:
                            flag_token_key = True
                            break
                    for index_target_role in range(len(words_lemma)):
                        for index_target_role_next in range(1, len(words_lemma)-index_target_role+1):
                            if ''.join(words_lemma[index_target_role: index_target_role + index_target_role_next]) == ''.join(target_role_tokens_lemma):
                                flag_token_key = True
                                break

                    if flag_token_key:
                        tokens_key.append(token_key)
                    
                if tokens_key != []:
                    verb_parser_dict_clean[key_] = {
                        'span': verb_parser_dict_clean_[key_]['span'],
                        'tokens': tokens_key.copy()
                    }
                            ...
                            for source_combination_sub_ in source_combination:
                                ...
                                if len(source_combination_sub) > 1:
                                    ...
                                    if not flag_have_shared_domain:
                                        break
                                # else: only have one list in source_cobination_sub don't need to find shared domain
                            if flag_have_shared_domain:
                                for source_combination_sub_ in source_combination:
                                    ...
                                    if len(source_combination_sub) == 1: # only have one argument
                                        ...
                                        for source_domain_shared in list(source_shared_domain): # may be have many source shared domain used the domain have minmum abstraction level as the fianl shared domain
